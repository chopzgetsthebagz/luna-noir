...
def generate_luna_image(scenario: str, nsfw: bool = False, width: int = 1536, height: int = 1536, seed: int = None) -> bytes:
    ...
    if nsfw:
        # NSFW prompts - emphasize REAL PHOTOGRAPHY and NATURAL HUMAN BODY
        full_prompt = f"{LUNA_BASE_DESCRIPTION}, {scenario}, professional nude photography, real human body, natural female form, realistic proportions, soft natural lighting, high detail skin texture, NSFW explicit, photorealistic, 8K resolution, uncensored"
    else:
        full_prompt = f"{LUNA_BASE_DESCRIPTION}, {scenario}, photorealistic, realistic skin pores, professional photography, masterpiece, 8K, sharp focus"
